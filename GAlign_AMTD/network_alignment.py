...
def parse_args():
    ...
    parser.add_argument('--source_dataset', default="graph_data/allmv_tmdb2/allmv/graphsage")
    parser.add_argument('--target_dataset', default="graph_data/allmv_tmdb2/tmdb/graphsage")
    parser.add_argument('--groundtruth',    default="graph_data/allmv_tmdb2/dictionaries/groundtruth")
...
def AttributeProcessing(G1,G2, alignment_dict):

    attribute_sim, attr1, attr2, attr1_pd, attr2_pd = read_attribute(G1, G2, alignment_dict)
...
def read_attribute(G1, G2, alignment_dict):
    try:
        attribute, attr1, attr2, attr1_pd, attr2_pd = load_attribute(G1, G2, alignment_dict)
...
def load_attribute(G1, G2, alignment_dict):

    G1_nodes = list(G1.nodes()) 
    G2_nodes = list(G2.nodes())
    
    attribute1_pd = pd.read_csv('dataset/attribute/am-tdattr1.csv', header = None, index_col = 0)
    attribute2_pd = pd.read_csv('dataset/attribute/am-tdattr2.csv', header = None, index_col = 0)
...
# ACN--------------------------------------------------------------
    ...
    G1_edges = pd.read_csv('dataset/graph/am-td1.edges', names = ['0', '1'])
    # print(G1_edges)
    ...
    G2_edges = pd.read_csv('dataset/graph/am-td2.edges', names = ['0', '1'])
    ...
    # alignment = pd.read_csv("dataset/alignment/am-td.csv", header = None)
...
# am-td -----------------------------------------------------------------------
    ...
    source_dataset = Dataset('dataset/DataProcessing/allmv_tmdb/allmv/graphsage')
    target_dataset = Dataset('dataset/DataProcessing/allmv_tmdb/tmdb/graphsage')
    # G1 = source_dataset.G
    # G2 = target_dataset.G 
    # print(G1.nodes())
    ...
    alignment_dict = gt_dict

    gt_dict = DeleteDuplicatedElement(gt_dict)

    attr1, attr2, attribute_sim  = AttributeProcessing(G1, G2, gt_dict)
...
# ACN--------------------------------------------------------------
    ...
    if algorithm == "IsoRank":
        ...
        model = GAlign(G1, G2, attr1, attr2, alignment_dict, idx1_dict, idx2_dict, source_dataset, target_dataset, args)
    elif algorithm == "PALE":
