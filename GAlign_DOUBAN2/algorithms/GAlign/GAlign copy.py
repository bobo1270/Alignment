...
class GAlign(NetworkAlignmentModel):
    ...
    def align(self):
        ...
        # noise graph
        target_A_hat = self.graph_augmentation(self.target_dataset, 'remove_edgse')
        # target_feats = self.graph_augmentation(self.target_dataset, 'change_feats')
    ...
    def multi_level_embed(self, source_A_hat, target_A_hat, source_feats, target_feats):
        ...
        for epoch in range(self.args.GAlign_epochs):
            ...
            for i in range(2):
                for j in range(len(new_source_A_hats)):
                    ...
                    if i == 0:
                        ...
                        outputs, loss_dgi_s = GAlign(source_A_hat, 's')
                        if j < 3:
                            augment_outputs,loss_dgi_ss = GAlign(augment_A_hat, 's')
                        else:
                            augment_outputs,loss_dgi_ss = GAlign(augment_A_hat, 's', new_source_feats)
                    else:
                        # Target loss
                        A_hat = target_A_hat
                        augment_A_hat = new_target_A_hats[j]
                        outputs,loss_dgi_s = GAlign(target_A_hat, 't')
                        if j < 3:
                            augment_outputs,loss_dgi_ss = GAlign(augment_A_hat, 't')
                        else:
                            augment_outputs,loss_dgi_ss = GAlign(augment_A_hat, 't', new_target_feats)
     
                    consistency_loss = self.linkpred_loss(outputs[-1], A_hat) # construst self loss
                    augment_consistency_loss = self.linkpred_loss(augment_outputs[-1], augment_A_hat) # construst augment self
                    # consistency_loss = self.args.beta * consistency_loss + (1-self.args.beta) * augment_consistency_loss # sum self and sugment self
                    # consistency_loss = self.args.beta * (consistency_loss+loss_dgi_s) + (1-self.args.beta) * (augment_consistency_loss + loss_dgi_ss)
                   
                    diff = torch.abs(outputs[-1] - augment_outputs[-1])
                    noise_adaptivity_loss = (diff[diff < self.args.threshold] ** 2).sum() / len(outputs)
                    # loss = self.args.coe_consistency * consistency_loss + (1 - self.args.coe_consistency) * noise_adaptivity_loss + 0.0005*(loss_dgi_s + loss_dgi_ss)
                    # loss = self.args.coe_consistency * consistency_loss + (1 - self.args.coe_consistency) * noise_adaptivity_loss + 0.64*(loss_dgi_s + loss_dgi_ss)
                    loss = self.args.coe_consistency * consistency_loss + (1 - self.args.coe_consistency) * noise_adaptivity_loss
                    # loss = self.args.coe_consistency * consistency_loss + 0.008*(loss_dgi_s)
                    # loss = self.args.coe_consistency * consistency_loss + (1 - self.args.coe_consistency) * noise_adaptivity_loss + 0.064*(loss_dgi_s + loss_dgi_ss)
            ...
            # # source and target graph together backward
            # outputs_S = GAlign(source_A_hat, 's')
            # outputs_T = GAlign(target_A_hat, 't')
            # s = torch.mean(outputs_S[-1], dim=0)
            # b = torch.mean(outputs_T[-1], dim=0)
            # loss_S_T = torch.norm(s - b)
            # # loss_S_T = torch.norm(s - b) + self.calculate_sim(outputs_S[-1], outputs_T[-1])

            # """loop out backward"""
            # print("out")
            # if self.args.log:
            #     print("Loss: {:.4f}".format(loss_S_T.data))
            # loss_S_T.backward()
            # structural_optimizer.step()
    ...
    # def calculate_sim(self,encoded_source,encoded_target):
    #     if encoded_source.shape[0] < encoded_target.shape[0]:
    #         encoded_source = self.data_add(encoded_source,encoded_target)
    #     else:
    #         encoded_target = self.data_add(encoded_target,encoded_source)
    #     # similarity_matrix = torch.sigmoid(torch.mm(encoded_source,encoded_target.t()))
    #     # loss_sim = -torch.norm(similarity_matrix)
    #     loss_sim = torch.norm(torch.mean(encoded_source,dim=0)-torch.mean(encoded_target,dim=0))

    #     return loss_sim


    # def data_add(self, small_data,big_data):
    ...
    def get_similarity_matrices(self, source_outputs, target_outputs):
        ...
        for i in range(len(source_outputs)):
            ...
            # # start
            # s_sum = torch.sum(source_output_i.square(), dim=1, keepdim=True).sqrt()
            # t_sum = torch.sum(target_output_i.square(), dim=1, keepdim=True).sqrt()
            # s_t_mo = torch.mm(s_sum, t_sum.t())
            # S = torch.mm(source_output_i, target_output_i.t()) / s_t_mo
            # # over
