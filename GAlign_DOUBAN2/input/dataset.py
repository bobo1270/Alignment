...
class Dataset:
    ...
    def __init__(self, data_dir):
        ...
        self._load_G()
        self._load_features()
        self.adj = graph_utils.construct_adjacency(self.G, self.id2idx, sparse=False, file_path=self.data_dir + "/edges.edgelist")
    ...
    def _load_G(self):
        ...
        G = nx.Graph()
        G_edges = pd.read_csv(open(os.path.join(self.data_dir, "douban.edges")), names = ['0', '1'])
        G_edges_str = G_edges.astype(str)
        G.add_edges_from(np.array(G_edges_str))
        self.G = G 
    ...
    def _load_features(self):
        ...
        if os.path.isfile(feats_path):
            ...
            self.features = np.zeros([self.G.number_of_nodes(), max_degree])
            # self.features = None
            for i in range(self.G.number_of_nodes()):
                try:
                    # note degree比in_degree效果好太多了
                    self.features[i][min(self.G.degree[i], max_degree-1)] = 1
                except:
                    self.features[i][0] = 1
