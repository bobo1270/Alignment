...
# ACN--------------------------------------------------------------
    ...
    # min_vals = np.min(S, axis=0)
    # max_vals = np.max(S, axis=0)
    # ranges = max_vals - min_vals
    # S = (S - min_vals) / ranges
    # matrix = []
    # for key,value in groundtruth.items():
    #     matrix.append(S[key,value])
    # a = sum(i >= 0.9 for i in matrix)
    # print(a)
    # # 绘制热力图
    # # plt.imshow(matrix, cmap='hot', interpolation='nearest')
    # # plt.colorbar()
    # matrix = sorted(matrix)
    # matrix = matrix[200:400]
    # plt.plot(matrix)
    # plt.savefig('/home/renpingsheng/ACN/GAlign_DOUBAN2/image.png')
